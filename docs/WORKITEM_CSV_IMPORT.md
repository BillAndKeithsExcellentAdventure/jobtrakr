# Cost Item CSV Import

## Overview

ProjectHound supports importing cost categories and cost items from CSV files. This feature allows you to quickly set up your cost tracking structure by importing predefined categories and line items from a properly formatted CSV file.
...
- **Cost item rows** contain only a cost item name and cost item code (category fields are empty)
...
| -------------- | ------------------ | ------ | ------------------------------------------------------------------------ |
| Category Name  | For category rows  | Text   | The name of the cost category (e.g., "Sitework", "Foundation")           |
| Category Code  | For category rows  | Number | Numeric code for the category (e.g., 100, 200)                           |
| Cost Item Name | For cost item rows | Text   | The name of the specific cost item (e.g., "Excavation", "Concrete Pour") |
| Cost Item Code | For cost item rows | Number | Numeric code for the cost item (e.g., 101, 102)                          |

### Format Rules

1. **Category Rows**: Must have values in both "Category Name" and "Category Code" columns. Cost item columns should be empty.
2. **Cost Item Rows**: Must have values in both "Cost Item Name" and "Cost Item Code" columns. Category columns should be empty.
...
4. Associates cost items with the current category
5. Returns an array of `WorkCategoryDefinition` objects with nested cost items
...
**Note:** While the internal code structure still uses the term "workItems" for the data property, these represent **cost items** from a user perspective.
...
4. Preview the parsed categories and cost items
5. Expand/collapse categories to review their cost items
6. Tap "Save" to import all categories and cost items
...
This file contains a comprehensive list of residential construction categories and cost items that can be used as a template or imported directly.
...
| ------------------ | ------------------------------------------- | ------------------------------------------------------- |
| Empty import       | CSV has no data rows after header           | Ensure CSV contains at least one category and cost item |
| Missing cost items | Category row not followed by cost item rows | Add cost item rows after each category row              |
...
- Number of cost items per category
...
2. **Descriptive Names**: Use clear, unambiguous names for categories and cost items
3. **Hierarchical Codes**: Consider using hierarchical codes (e.g., 10, 15, 20 for parent and children - these codes are often displayed as costCategory.costItem i.e., '10.15' for Sitework-Excavation)
4. **Preview Before Saving**: Always review the parsed data before committing the import
5. **Backup**: Export existing cost items before importing new ones if you want to preserve current data
