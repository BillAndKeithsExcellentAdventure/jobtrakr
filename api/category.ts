...
export function fetchCategoriesForJob(projectId: number): Promise<JobCategoryEntry[]> {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      // Simulating a successful data load with a slight delay
      const success = true; // You can toggle this to simulate errors
      if (success) {
        const specifiedJobCategories = ALLJOBCATEGORYDATA.filter((e) => e.projectId === projectId);
